        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($instance)
                ...
                ->addColumn('actions', function ($row) {
                    $status = $row->status === 1 ?
                        '<a href="#!" data-url="' . route('admin.live.unpublish', $row->id) . '" class="btn-toggle dropdown-item">Unpublish</a>' :
                        '<a href="#!" data-url="' . route('admin.live.publish', $row->id) . '" class="btn-toggle dropdown-item">Publish</a>';
                    ...
                    return
                        '<div class="dropdown">
									    ...
									    <div class="dropdown-menu dropdown-menu-right">
											<a href="#!" data-url-edit="' . route('admin.live.edit', $row->id) . '" data-url-update="' . route('admin.live.update', $row->id) . '" data-toggle="modal" data-target="#edit-modal" class="dropdown-item">Edit</a>
											<a href="#!" data-url="' . route('admin.live.destroy', $row->id) . '" class="btn-delete dropdown-item">Delete</a>
											' . $status . '
									    </div>
									</div>';
                })
