...
class SportController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            // $instance = $this->channel->sports()->latest();
            ...
            return DataTables::of($instance)
                ...
                ->addColumn('actions', function ($row) {
                    $status = $row->status === 1 ?
                        '<a href="#!" data-url="' . route('channel.sports.unpublish', $row->id) . '" class="btn-toggle dropdown-item">Unpublish</a>' :
                        '<a href="#!" data-url="' . route('channel.sports.publish', $row->id) . '" class="btn-toggle dropdown-item">Publish</a>';
                    ...
                    return
                        '<div class="dropdown">
									    ...
									    <div class="dropdown-menu dropdown-menu-right">
											<a href="#!" data-url-edit="' . route('channel.sports.edit', $row->id) . '" data-url-update="' . route('channel.sports.update', $row->id) . '" data-toggle="modal" data-target="#edit-modal" class="dropdown-item">Edit</a>
											<a href="#!" data-url="' . route('channel.sports.destroy', $row->id) . '" class="btn-delete dropdown-item">Delete</a>
											' . $status . '
									    </div>
									</div>';
                })
        ...
        return view('channel.sports.index');
    }
