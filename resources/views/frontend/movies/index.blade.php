...
@section('content')
    <div class="container-fluid" style="padding-top: 80px;">
        <div class="d-flex justify-content-between align-items-center mt-4 mb-4">
            <h4>Movies</h4>
            <form action="{{ route('frontend.movies.index') }}" method="GET">
        ...
        @if(isset($genres) && $genres->isNotEmpty())
            @if(request()->has('keywords') && request()->keywords)
                <div class="mb-4">
                    Found <strong>{{ $genres->pluck('movies')->first()->count() }}</strong> results,
            ...
            @foreach($genres as $genre)
                <div class="d-flex justify-content-between">
                    ...
                    <a href="{{ route('frontend.movies.genres', $genre->id) }}" class="ml-3 cursor-pointer">
                        View all <i class="fas fa-arrow-circle-right"></i>
                    </a>
                </div>

                @if(isset($genre->movies) && $genre->movies->isNotEmpty())
