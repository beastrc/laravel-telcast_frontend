...
@section('content')
    <div class="container-fluid" style="padding-top: 80px;">
        <div class="d-flex justify-content-between align-items-center mt-4 mb-4">
            <h4>Shows <i class="fas fa-angle-right fa-sm"></i> {{ $genre->name }}</h4>
            <form action="{{ route('frontend.shows.genres', $genre->id) }}" method="GET">
        ...
        @if(isset($shows) && $shows->isNotEmpty())
            @if(request()->has('keywords') && request()->keywords)
                <div class="mb-4">
                    Found <strong>{{ $shows->total() }}</strong> results in <strong>{{ $shows->lastPage() }}</strong>
                    pages,
