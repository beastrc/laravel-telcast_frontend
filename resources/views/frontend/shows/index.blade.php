...
@section('content')
    <div class="container-fluid" style="padding-top: 80px;">
        <div class="d-flex justify-content-between align-items-center mt-4 mb-4">
            <h4>Shows</h4>
            <form action="{{ route('frontend.shows.index') }}" method="GET">
        ...
        @if(isset($genres) && $genres->isNotEmpty())
            @if(request()->has('keywords') && request()->keywords)
                <div class="mb-4">
                    Found <strong>{{ $genres->pluck('shows')->first()->count() }}</strong> results,
            ...
            @foreach($genres as $genre)
                <div class="d-flex justify-content-between">
                    ...
                    <a href="{{ route('frontend.shows.genres', $genre->id) }}" class="ml-3 cursor-pointer">
                        View all <i class="fas fa-arrow-circle-right"></i>
                    </a>
                </div>

                @if(isset($genre->shows) && $genre->shows->isNotEmpty())
                    <ul class="list-inline row p-0 mb-5 iq-rtl-direction mx-0">
                        @foreach($genre->shows as $item)
                            <li class="slide-item">
