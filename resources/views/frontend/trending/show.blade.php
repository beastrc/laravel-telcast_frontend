...
@section('content')
    <div class="container-fluid" style="padding-top: 80px;">
        <div class="d-flex justify-content-between align-items-center mt-4 mb-4">
            <h4>{{ $genre->name }}</h4>
            <form action="{{ route('frontend.trending.show', $genre->id) }}" method="GET">
        ...
        @if(isset($trending) && $trending->isNotEmpty())
            @if(request()->has('keywords') && request()->keywords)
                <div class="mb-4">
                    Found <strong>{{ $trending->total() }}</strong> results in
                    <strong>{{ $trending->lastPage() }}</strong>
                    pages,
        ...
        @elseif(request()->has('keywords') && request()->keywords)
            <div class="mb-4">
                Found <strong>{{ $trending->total() }}</strong> results in <strong>{{ $trending->lastPage() }}</strong>
                pages,
                matching <strong>{{ request()->keywords }}</strong>
            </div>
        @else
