...
@section('content')
    <div class="container-fluid" style="padding-top: 80px;">
        <div class="d-flex justify-content-between align-items-center mt-4 mb-4">
            <h4>Trending</h4>
            <form action="{{ route('frontend.trending.index') }}" method="GET">
        ...
        @if(isset($genres) && $genres->isNotEmpty())
            @if(request()->has('keywords') && request()->keywords)
                <div class="mb-4">
                    Found <strong>{{ $genres->total() }}</strong> results in
                    <strong>{{ $genres->lastPage() }}</strong>
                    pages,
            ...
            @foreach($genres as $genre)
                <div class="d-flex justify-content-between">
                    ...
                    <a href="{{ route('frontend.trending.show', $genre->id) }}" class="ml-3 cursor-pointer">
                        View all <i class="fas fa-arrow-circle-right"></i>
                    </a>
                </div>
                <ul class="list-inline row p-0 mb-5 iq-rtl-direction mx-0">
                    @foreach($genre->getRelations() as $relation => $items)
        ...
        @elseif(request()->has('keywords') && request()->keywords)
            <div class="mb-4">
                Found <strong>{{ $genres->total() }}</strong> results in <strong>{{ $genres->lastPage() }}</strong>
                pages,
                matching <strong>{{ request()->keywords }}</strong>
            </div>
        @else
