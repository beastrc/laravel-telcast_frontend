		...
		@if(isset($genres) && $genres->isNotEmpty())
			@foreach($genres as $genre)
				@if($genre->children()->exists())
					@include('channel.genres.partials.children', ['genre' => $genre])
				@else
								...
								<div class="d-flex align-items-center justify-content-between">
									...
									<span class="right">
										<i class="btn-edit fas fa-pencil-alt mr-1"
										   data-url-edit="{{ route('channel.genres.edit', $genre->id) }}"
										   data-url-update="{{ route('channel.genres.update', $genre->id) }}"></i>
										<i class="btn-delete fas fa-times fa-lg text-danger mr-1"
										   data-url="{{ route('channel.genres.destroy', $genre->id) }}"></i>
									</span>
								</div>
